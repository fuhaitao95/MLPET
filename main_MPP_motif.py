...
from Prompt_model.Predictor import DDI_Predictor, SMILES_Predictor, Motify_SMILES_Predictor
...
from Utils.featureUtil import atomic_nums_to_smiles_tokens, convert_smiles_to_single_char
...
from dataset.motif_moleculeDataset.dataset_motif import PygGraphPropPredDataset_motif
...
def train(model, device, train_loader, optimizer, converter, learnable_prompt, prompt_tuning_module, smiles_predictor,
          task_type):
    model.train()
    total_loss = 0
    num_batches = 0
    ...
    for batch in tqdm(train_loader, desc="Training", leave=False):
        ...
        batch.smiles = [convert_smiles_to_single_char(smi) for smi in batch.smiles]
        encoded = converter(batch.smiles).to(device)
        # smiles_atom = atomic_nums_to_smiles_tokens(batch.x, batch.batch)
        # encoded = converter(smiles_atom).to(device)

        # todo [32,81] tensor 识别其中的原子位置mask
        atom_mask = [
            torch.tensor(converter.code2atomadj(toks.to(device)), dtype=torch.bool)
            for toks in encoded
        ]
        atom_mask = torch.stack(atom_mask, dim=0).to(device)  # [B, N]
        ...
        with autocast():  # 自动混合精度（减少显存占用 + 更快）
            ...
            y_predict = smiles_predictor(logits, atom_mask, batch)
...
def eval(model, device, loader, converter, learnable_prompt, prompt_tuning_module, smiles_predictor, evaluator):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Evaluating", leave=False):
            ...
            # # 打印每个分子的对比信息
            # for i in range(len(smiles_atom)):
            #     mask_count = atom_mask[i].sum().item()
            #     smiles_atom_count = len(smiles_atom[i])
            #     print(
            #         f"Sample {i}: atom_mask True count = {mask_count}, SMILES atom count = {smiles_atom_count}, SMILES = {smiles_atom[i]}")
...
def main():
    ...
    parser.add_argument('--predictor_attention_head_num', type=int, default=16, help="20 heads in paper")
    ...
    parser.add_argument('--model_load_path', type=str,
                        default="save/Molecule_pretrain-D_QM9SMILES-L_5-A_16-DIM_256-B_64-LR_0.0001-E_100-20241211-1517/model.pth")
    ...
    dataset = PygGraphPropPredDataset_motif(root="dataset/motif_moleculeDataset",
                                            name=args.dataset)
    ...
    motif_smiles_predictor = Motify_SMILES_Predictor(atom_feature_size=args.embed_dim,
                                                     num_tasks=dataset.num_tasks, num_layer=args.predictor_layer_num,
                                                     emb_dim=args.embed_dim, drop_ratio=args.dropout).to(device)
    # 初始化优化器，包含所有需要优化的模块参数
    optimizer = Adam(
        list(model.parameters()) +
        list(prompt_tuning_module.parameters()) +  # todo 是否固定
        list(motif_smiles_predictor.parameters()),
        lr=args.lr,
        weight_decay=1e-4
    )

    # >>> 添加学习率调度器（每 100 epoch 学习率乘以 0.5）
    ...
    if args.model_training_save_path != '':
        ...
        if len(checkpoints) > 0:
            ...
            motif_smiles_predictor.load_state_dict(checkpoint['smiles_predictor_state_dict'])
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        avg_loss = train(model, device, train_loader, optimizer, converter, learnable_prompt, prompt_tuning_module,
                         motif_smiles_predictor, dataset.task_type)
        ...
        test_results = eval(model, device, test_loader, converter, learnable_prompt,
                            prompt_tuning_module, motif_smiles_predictor, evaluator)
        ...
        if args.model_training_save_path != '' and epoch % 50 == 0:
            ...
            checkpoint = {
                ...
                'smiles_predictor_state_dict': motif_smiles_predictor.state_dict(),
            }
    ...
    if not args.model_save_path == '':
        ...
        torch.save(motif_smiles_predictor, path)
