...
from Prompt_model.Predictor import DDI_Predictor, SMILES_Predictor, SMILES_Graph_Predictor
...
def train(model, device, train_loader, optimizer, converter, learnable_prompt, prompt_tuning_module, smiles_predictor,
    ...
    for batch in tqdm(train_loader, desc="Training", leave=False):
        ...
        with autocast():  # 自动混合精度（减少显存占用 + 更快）
            ...
            y_predict = smiles_predictor(logits, batch.x, atom_mask, batch)
...
def main():
    ...
    smiles_predictor = SMILES_Graph_Predictor(args, dataset.num_tasks).to(device)
