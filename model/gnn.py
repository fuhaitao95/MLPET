...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_tasks, num_layer=5, emb_dim=300,
                 gnn_type='gin', virtual_node=True, residual=False, drop_ratio=0.5, JK="last",
...
class GNN_woAll(torch.nn.Module):
    ...
    def forward(self, batched_data):
        h_node = self.motif_encoder(batched_data)

        h_graph_node = self.graph_encoder(
            [h_node, batched_data.edge_index, batched_data.edge_attr])

        h_graph = self.pool(h_graph_node, batched_data.batch)
